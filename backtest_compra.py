"""
Backtest — COMPRA quando WIN cai >= 500 pts do open (09:00-10:30)
...
CONTRATOS     = 1
NIVEL_ENTRADA = -500   # WIN cai 500 pts do open -> COMPRA
...
# ── Funcao de backtest ─────────────────────────────────────────
def backtest(nivel, gain_pts, stop_pts, direcao='COMPRA'):
    trades = []
    ...
    for dia in dias:
        ...
        preco_alvo = open_0900 + nivel  # nivel < 0 = queda
...
print("="*75)
print(f"GRID SEARCH — COMPRA em -{abs(NIVEL_ENTRADA)} pts do open")
...
for gain, stop in product(GAINS, STOPS):
    df_t = backtest(NIVEL_ENTRADA, gain, stop, 'COMPRA')
    if len(df_t) < 20:
        continue
...
if len(top5) > 0:
    ...
    df_best = backtest(NIVEL_ENTRADA, g_best, s_best, 'COMPRA')
    ...
    print(f"\n  Por horario de entrada:")
    print(f"  {'Faixa':>12}  {'N':>5}  {'WR%':>7}  {'P&L medio':>12}")
    for faixa, faixa2, label in [(0,20,'09:00-09:20'),(20,40,'09:20-09:40'),(40,60,'09:40-10:00'),(60,90,'10:00-10:30')]:
        ...
        med  = g['pnl_brl'].mean()
        print(f"  {label:>12}  {len(g):>5}  {wr_h:>6.1f}%  R${med:>9,.1f}")
    ...
    dd    = (eq - eq.cummax()).min()
    gross_w = df_best[df_best['pnl_brl']>0]['pnl_brl'].sum()
    gross_l = abs(df_best[df_best['pnl_brl']<0]['pnl_brl'].sum())
    pf    = gross_w / gross_l if gross_l > 0 else 99
    ...
    df_best.to_csv(f"{RESULTS_DIR}/backtest_compra_500.csv", index=False)
    print(f"\n  Trades salvos: {RESULTS_DIR}/backtest_compra_500.csv")
