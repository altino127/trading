import pandas as pd
import numpy as np
from config import JANELA_BETA_DIAS, JANELA_PEER_DIAS, ZSCORE_ENTRADA_BULL, ZSCORE_PEER_BEAR
...
def snapshot_atual(
    ...
    if zscores_peer is not None and not zscores_peer.empty:
        peer_atual = zscores_peer.iloc[-1].dropna()
        df["zscore_peer"] = peer_atual.reindex(df.index)
    else:
        df["zscore_peer"] = np.nan

    df = df.sort_values("zscore")

    # flags para cada modo
